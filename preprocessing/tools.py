...
def feat_eeg(signals, feature_extraction = True, normalization = False):
    """
    ...
    if(feature_extraction):
        ...
        features.append(spindle)

    sum_abs_pow = delta + theta + alpha + beta + gamma + spindle
    if(normalization):
        features.append(delta/sum_abs_pow)
        features.append(theta/sum_abs_pow)
        features.append(alpha/sum_abs_pow)
        features.append(beta/sum_abs_pow)
        features.append(gamma/sum_abs_pow)
        features.append(spindle/sum_abs_pow)
    
    Power_Ratio = [delta/sum_abs_pow, theta/sum_abs_pow, alpha/sum_abs_pow, beta/sum_abs_pow, gamma/sum_abs_pow, spindle/sum_abs_pow]
    
    spectral_entropy = spectral_entropy2(Power_Ratio)
    features.append(spectral_entropy)
    
    svd_entropy = svd_entropy2(signals, 1, 4)
    features.append(svd_entropy)
    
    kurtosis = np.log10(stats.kurtosis(signals, fisher=False, axis=1)) 
    features.append(kurtosis)
    
    features = np.array(features)
    
    if np.any(features==np.nan): print('NaN detected')
    return np.nan_to_num(features)

    
def feat_eeg2(signals, feature_extraction = True, normalization = False, sfreq = 100):
    """
    calculate the relative power as defined by Leangkvist (2012),
    assuming signal is recorded with 100hz
    """
    ...
    for ii in range(num_slice):
        ...
        data = signals[ii*sfreq:(ii+1)*sfreq]
        print(data)
        bands = [0.5,4,7,12,30]
        #bin_power(signals[ii*sfreq:(ii+1)*sfreq], bands, 1)
...
def feat_wavelet(signals):
    ...
    sfreq = use_sfreq
    nsamp = float(signals.shape[1])
    feats = np.zeros((signals.shape[0],8+5),dtype='float32')
...
def feat_eog(signals):
    ...
    nsamp = float(signals.shape[1])
    w = (fft(signals,axis=1)).real   
    feats = np.zeros((signals.shape[0],15+5),dtype='float32')
    delta = np.sum(np.abs(w[:,np.arange(0.5*nsamp/sfreq,4*nsamp/sfreq, dtype=int)]),axis=1)
    theta = np.sum(np.abs(w[:,np.arange(4*nsamp/sfreq,8*nsamp/sfreq, dtype=int)]),axis=1)
    alpha = np.sum(np.abs(w[:,np.arange(8*nsamp/sfreq,13*nsamp/sfreq, dtype=int)]),axis=1)
    beta  = np.sum(np.abs(w[:,np.arange(13*nsamp/sfreq,20*nsamp/sfreq, dtype=int)]),axis=1)
    ...
    feats[:,12] = np.std(signals, axis=1) #STD/VAR
    feats[:,13] = stats.kurtosis(signals,fisher=False,axis=1)       # kurtosis
    feats[:,14] = -np.sum([(x/nsamp)*((np.log((x+np.spacing(1))/nsamp))) for x in np.apply_along_axis(lambda x: np.histogram(x, bins=8)[0], 1, signals)],axis=1)  # entropy.. yay, one line...
...
def feat_emg(signals, sfreq=100):
    """
    ...
    nsamp = float(signals.shape[1])
    w = (fft(signals,axis=1)).real   
    feats = np.zeros((signals.shape[0],13+5),dtype='float32')
    delta = np.sum(np.abs(w[:,np.arange(0.5*nsamp/sfreq,4*nsamp/sfreq, dtype=int)]),axis=1)
    theta = np.sum(np.abs(w[:,np.arange(4*nsamp/sfreq,8*nsamp/sfreq, dtype=int)]),axis=1)
    alpha = np.sum(np.abs(w[:,np.arange(8*nsamp/sfreq,13*nsamp/sfreq, dtype=int)]),axis=1)
    beta  = np.sum(np.abs(w[:,np.arange(13*nsamp/sfreq,20*nsamp/sfreq, dtype=int)]),axis=1)
...
# Applying discrete WT on each channel
def multch_wavelet_single_channel(channel, wavelet='db4', mode=pywt.MODES.zpd, numLevels=2):
    #coeffs = list()
    #for i, channel in enumerate(data):
    #    coeffs.append(pywt.wavedec(channel, wavelet, mode, numLevels))
    return pywt.wavedec(channel, wavelet, mode, numLevels)
...
def spectral_entropy2(Power_Ratio):
    Spectral_Entropy = 0
...
def svd_entropy2(X, Tau, DE, W=None):
